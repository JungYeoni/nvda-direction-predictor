...
초기 실험에서는 `NVDA가 다음 거래일에 상승하는가`를 타깃으로 두었다. 그러나 일별 주가에는 작은 잡음성 상승과 하락이 많아, 방향 자체를 맞히는 문제는 예측력이 제한적이었다. 이에 최종 모형에서는 다음과 같이 타깃을 재정의했다.

```text
target = 1 if NVDA 일별 수익률 - QQQ 일별 수익률 > 0.2%p
         0 otherwise
```

즉, 최종 모형은 "NVDA가 단순히 올랐는가"가 아니라 "NVDA가 QQQ보다 의미 있게 강했는가"를 예측한다. 이 정의는 NVDA의 절대 방향보다 시장 대비 초과 성과를 평가하는 데 적합하며, 실제 투자 관점에서는 NVDA overweight 또는 long NVDA / hedge QQQ 전략과 연결된다.
...
셋째, 개선 가능성이 가장 높은 Logistic Regression과 XGBoost를 우선 튜닝했다. 최종 선택 기준은 validation ROC-AUC이며, classification threshold는 validation balanced accuracy가 최대가 되는 값으로 선택했다. 최종 선택된 모형은 tuned Logistic Regression이다.
...
|------|------|------|--------|----------|-----------|--------|----|---------|-----|
| 최종 모형 | NVDA - QQQ > 0.2%p | Tuned LR | 225 | **58.22%** | **0.547** | 0.505 | 0.525 | 0.603 | **0.154** |
| 기준 모형 | NVDA return > 0 | LR | 225 | 53.33% | 0.547 | 0.636 | 0.588 | 0.558 | 0.058 |
| 비교 모형 1 | NVDA - QQQ > 0.2%p | LR | 225 | 57.33% | 0.531 | 0.583 | 0.556 | **0.610** | 0.148 |
| 비교 모형 2 | NVDA - QQQ > 0 | XGBoost | 225 | 56.00% | 0.628 | 0.246 | 0.353 | 0.601 | 0.135 |
| 비교 모형 3 | NVDA - QQQ > 0.2%p | LSTM | 205 | 54.15% | 0.507 | 0.792 | 0.618 | 0.594 | 0.127 |
| 비교 모형 4 | NVDA return > 0 | GRU | 205 | 53.66% | 0.554 | 0.661 | 0.603 | 0.572 | 0.058 |

최종 tuned LR은 Accuracy 58.22%를 기록했다. 과제 기준상 58% 이상 구간에 해당하므로 예측력 평가에서 가장 높은 구간을 목표로 제시할 수 있다. Precision은 0.547로, 모형이 "NVDA가 QQQ보다 0.2%p 이상 강하다"고 예측한 날 중 약 54.7%가 실제로 맞았다.

ROC-AUC만 보면 튜닝 전 LR이 0.610으로 가장 높았지만, 튜닝 LR은 validation 기준으로 선택한 threshold에서 Accuracy와 MCC가 개선되었다. 따라서 발표에서는 최종 실전형 모형을 tuned LR로 제시하고, ROC-AUC 관점의 비교 결과도 함께 설명하는 것이 적절하다.

### 그림 1. 실제 주가와 예측 정오 표시

![Figure 1. NVDA close with prediction correctness](figures/final_assignment_prediction_correctness.png)

그림 1은 test 기간의 NVDA 종가 추세 위에 최종 모형의 예측이 맞은 날과 틀린 날을 표시한 것이다. 위 패널은 실제 NVDA 종가이며, 초록색 점은 예측 성공일, 빨간색 x는 예측 실패일이다. 아래 패널은 실제 target과 예측 target을 0/1 선으로 표시한다. 이를 통해 단순 수치뿐 아니라 어느 구간에서 예측이 잘 맞고 틀렸는지 시각적으로 확인할 수 있다.

## 5. 비교 분석

본 과제 요구사항에 맞춰 두 가지 비교를 수행했다.

첫째, 타깃 정의를 비교했다. 기존의 `NVDA return > 0` 타깃은 LR 기준 Accuracy 53.33%, ROC-AUC 0.558에 그쳤다. 반면 `NVDA return - QQQ return > 0.2%p` 타깃은 tuned LR 기준 Accuracy 58.22%, ROC-AUC 0.603을 기록했다. 이는 NVDA의 절대 방향보다 시장 대비 상대 강도를 예측하는 문제가 더 안정적이라는 점을 보여준다.
...
최종적으로 본 프로젝트는 NVDA의 단순 상승/하락 예측보다 QQQ 대비 초과 성과 예측이 더 높은 예측력을 보인다는 결론을 얻었다. 최종 tuned Logistic Regression 모형은 test Accuracy 58.22%, Precision 0.547, ROC-AUC 0.603을 기록했다. 이는 단순 majority baseline 54.22%를 상회하며, 과제 예측력 기준에서도 58% 이상 구간에 해당한다.

발표에서는 "복잡한 딥러닝보다 단순하지만 잘 정의된 타깃과 시장 구조 반영 변수가 더 중요했다"는 메시지를 중심으로 설명하는 것이 좋다. 즉, 이 프로젝트의 핵심 기여는 더 복잡한 모형을 만드는 것보다, NVDA의 움직임을 시장 대비 상대 강도 문제로 재정의해 예측 가능성을 높였다는 점이다.
...
| 2:30~3:40 | 표 1 결과 | 최종 tuned LR이 Accuracy 58.22%, Precision 0.547로 가장 실용적인 성능을 보였다. |
